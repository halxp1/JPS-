...
void Jps::PrintAllRoute() {
    Getallpath();
    cout<<endl<<"allroute"<<"("<<AllPath.size()<<"):";
    for(int i=0; i<AllPath.size();i++){
        cout<<AllPath[i].first<<","<<AllPath[i].second<<" ";
    }
    cout<<endl;
}

vector<std::pair<int, int>> Jps::Calabs(PathNode &node1, PathNode &node2) {
    int diff = 0;
    int stx=node1.row;
    int sty=node1.col;
    int enx=node2.row;
    ...
    if(abs(enx - stx)>1 || abs(eny - sty)>1){
        if(enx == stx && eny != sty){     //水平方向
            diff = eny - sty;
            if(diff>0) {
                for(int i=sty+1; i<=sty+diff; i++)
                    result.push_back(std::make_pair(stx, i));     //水平向左
            }
            else if(diff<0) {
                for(int i =sty-1; i>= sty+diff; i--)
                    result.push_back(std::make_pair(stx,i));      //水平向右
            }

        }
        else if(stx != enx && sty == eny){                       //垂直方向
            diff = stx - enx;
            if(diff>0){
                for(int i = stx-1; i>=stx-diff; i++)              //垂直向上
                    result.push_back(std::make_pair(i, sty));
            }
            else if(diff<0){
                for(int i = stx+1; i<=stx+diff; i++)             //垂直向下
                    result.push_back(std::make_pair(i, sty));
            }
        }
        else if(stx>enx && sty<eny){                             //第一象限
            diff = stx - enx;
            for(int i =1;i<=diff; i++)
                result.push_back((std::make_pair(stx-i, sty+i)));
        }
        else if(stx<enx && sty>eny){                            //第三象限
            diff = enx - stx;
            for(int i=1; i<=diff; i++)
                result.push_back(std::make_pair(stx+i, sty-i));
        }
        else if(stx < enx && sty< eny){                         //第二象限
            diff = enx -stx;
            for(int i=1; i<=diff; i++)
                result.push_back(std::make_pair(stx-i, sty-i));
        }
        else if(stx>enx && sty> eny){                          //第四象限
            diff = stx-enx;
            for(int i=1; i<=diff; i++)
                result.push_back(std::make_pair(stx+i, sty+i));
        }
        ...
        result.push_back(std::make_pair(enx, eny));
...
void Jps::Getallpath() {
    int len = retPath.size();
    AllPath.push_back(std::make_pair(retPath[0].row, retPath[0].col));  //起始点
    for(int i=1; i<len; i++){
        vector<std::pair<int, int>> result = Calabs(retPath[i-1],retPath[i]);
        AllPath.insert(AllPath.end(), result.begin(), result.end());
    }
}
void Jps::UpdateMap() {
    nullNode.isnull = true;
    for(int i=0; i<retPath.size(); i++){
        pathMap[retPath[i].row][retPath[i].col]->parent = nullptr;
        pathMap[retPath[i].row][retPath[i].col]->isfind = false;
        pathMap[retPath[i].row][retPath[i].col]->isroute = false;
        pathMap[retPath[i].row][retPath[i].col]->inopen = false;
        pathMap[retPath[i].row][retPath[i].col]->f = 0;
        pathMap[retPath[i].row][retPath[i].col]->g = 0;
        pathMap[retPath[i].row][retPath[i].col]->h = 0;
    }
    retPath.clear();
    AllPath.clear();
}
